 ...
 * \tparam fake_tso Omit generation of explicit barrier code to avoid unnecesary
...
template <typename T, size_t buffer_size = 16, bool fake_tso = false,
          size_t cacheline_size = 64, typename index_t = size_t>
class Ringbuffer {
public:
    /*!
    ...
    /*!
     * \brief Special case constructor to premature out unnecessary
     * initialization code when object is instatiated in .bss section \warning
     * If object is instantiated on stack, heap or inside noinit section then
     * the contents have to be explicitly cleared before use \param dummy
     * Ignored
     */
    ...
    {
        // head modification will lead to underflow if cleared during consumer
        // read doing this properly with CAS is not possible without modifying
        // the consumer code
        consumerClear();
    }
     ...
     * into internal buffer, is located \return True if data was inserted
     */
     ...
     *
     * \param get_data_callback Pointer to callback function that returns
     * element to be inserted into buffer \return True if data was inserted and
     * callback called
     */
    ...
    bool remove()
    {
        ...
        if (tmp_tail == head.load(std::memory_order_relaxed))
            ...
            tail.store(++tmp_tail,
                       index_release_barrier); // release in case data was
                                               // loaded/used before
     ...
     * \param[out] data Reference to memory location where removed element will
     * be stored \return True if data was fetched from the internal buffer
     */
    bool remove(T& data)
    {
     ...
     * stored \return True if data was fetched from the internal buffer
     */
    bool remove(T* data)
    {
        index_t tmp_tail = tail.load(std::memory_order_relaxed);
     ...
     * \brief Gets the n'th element on consumed side
     *
     * Unchecked operation, assumes that software already knows if the element
     * can be used, if requested index is out of bounds then reference will
     * point to somewhere inside the buffer The isEmpty() and readAvailable()
     * will place appropriate memory barriers if used as loop limiter It is safe
     * to use and modify T contents only on consumer side
     *
     * \param index Item offset starting on the consumed side
     * \return Reference to requested element, undefined if index exceeds
     * storage count
     */
     ...
     * or there is no more space. The callback function can be used to indicate
     * to consumer that it can start fetching data.
     *
     ...
     * callback function in first loop \param execute_data_callback Pointer to
     ...
     * \brief Load multiple elements from internal buffer without blocking
     *
     * This function will continue reading new entries until all requested data
     * is read or there is nothing more to read. The callback function can be
     * used to indicate to producer that it can start writing new data.
     *
     * \warning This function is not deterministic
     *
     * \param[out] buff Pointer to buffer where data will be loaded into
     ...
     * callback function in first iteration \param execute_data_callback Pointer
     * to callback function executed after every loop iteration \return Number
     * of elements that were read from internal buffer
     */
...
private:
    ...
    constexpr static std::memory_order index_acquire_barrier =
        fake_tso ? std::memory_order_relaxed
                 : std::memory_order_acquire; // do not load from, or store to
                                              // buffer before confirmed by the
                                              // opposite side
    ...
    // put buffer after variables so everything can be reached with short
    // offsets
    ...
    static_assert(!(std::numeric_limits<index_t>::is_signed),
                  "indexing type shall not be signed");
